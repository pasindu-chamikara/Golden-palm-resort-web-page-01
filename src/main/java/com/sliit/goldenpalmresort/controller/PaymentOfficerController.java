...
import com.sliit.goldenpalmresort.repository.PaymentRefundRepository;
...
@PreAuthorize("hasAnyRole('PAYMENT_OFFICER','MANAGER','ADMIN')")
public class PaymentOfficerController {
    ...
    public ResponseEntity<PaymentResponse> getPaymentById(@PathVariable Long id) {
        try {
			...
			PaymentResponse response = new PaymentResponse(payment);
			BigDecimal alreadyRefunded = paymentRefundRepository.sumRefundsByPaymentId(id);
			if (alreadyRefunded == null) alreadyRefunded = BigDecimal.ZERO;
			BigDecimal remaining = payment.getAmount() != null ? payment.getAmount().subtract(alreadyRefunded) : BigDecimal.ZERO;
    ...
    public ResponseEntity<Map<String, Object>> processRefund(
        ...
        try {
			...
			// Validate against remaining refundable (cumulative partial refunds)
			BigDecimal alreadyRefunded = paymentRefundRepository.sumRefundsByPaymentId(id);
			if (alreadyRefunded == null) alreadyRefunded = BigDecimal.ZERO;
			...
			if (refundAmount.compareTo(remaining) > 0) {
				System.err.println("Refund amount exceeds remaining refundable amount");
				return ResponseEntity.badRequest().body(Map.of("error", "Refund amount cannot exceed remaining refundable amount",
			...
			response.put("remainingRefundable", paymentAmount.subtract(newTotalRefunded));
	...
	public ResponseEntity<List<Map<String, Object>>> getRefundHistory(@PathVariable Long id) {
		try {
			Payment payment = paymentRepository.findById(id).orElse(null);
			if (payment == null) {
				return ResponseEntity.notFound().build();
			}
			List<PaymentRefund> refunds = paymentRefundRepository.findByPaymentIdOrderByRefundDateDesc(id);
			List<Map<String, Object>> response = new ArrayList<>();
			for (PaymentRefund r : refunds) {
				Map<String, Object> item = new HashMap<>();
				item.put("amount", r.getAmount());
				item.put("reason", r.getReason());
				item.put("processedBy", r.getProcessedBy());
				item.put("refundDate", r.getRefundDate());
				response.add(item);
			}
			return ResponseEntity.ok(response);
		} catch (Exception e) {
			System.err.println("Error in getRefundHistory: " + e.getMessage());
			e.printStackTrace();
			return ResponseEntity.internalServerError().build();
		}
	}
    ...
    public ResponseEntity<List<PaymentResponse>> getPaymentsByStatus(@PathVariable String status) {
        try {
            ...
            for (Payment payment : payments) {
                try {
					PaymentResponse response = new PaymentResponse(payment);
					BigDecimal alreadyRefunded = paymentRefundRepository.sumRefundsByPaymentId(payment.getId());
					if (alreadyRefunded == null) alreadyRefunded = BigDecimal.ZERO;
					BigDecimal remaining = payment.getAmount() != null ? payment.getAmount().subtract(alreadyRefunded) : BigDecimal.ZERO;
					response.setRemainingRefundable(remaining);
                    paymentResponses.add(response);
